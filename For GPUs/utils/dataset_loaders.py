...
class FAST_Imagenet(torchvision.datasets.ImageFolder):
    '''300 iter / s! fastest fr'''
    def __init__(self,
                 src,
                 fast_src = None,
        ...
        '''Constructor for the pretraining dataset
        
        Arguments:
        1. src: the source imagenet dataset train dir path
        2. fast_src: the dir for the FAST components
        3. resized_crop_params: dictionary containing scale and ratio as keys
        4. dimension: the dimension of the resized image
        5. hflip_prob: probabilty to conduct horizontal flipping with
        6. vflip_prob: probability to conduct vertical flipping with
        7. normalization: post conversion normalization for base images
        8. mask_patch_size: the mask generator patch size for SimMIM
        9. model_patch_size: the patch size of the model inputs
        10. mask_ratio: the masking ratio for the patches'''
        super().__init__(src)
        self.src = src
...
def load_imagenet(root_directory):
    '''Function to load a truncated version of the ImageNet dataset.
    
    Arguments:
    1. root_directory: the root directory in which the dataset is stored
    ...
    train_dataset = torchvision.datasets.ImageNet(root_directory, split = 'train')
    test_dataset   = torchvision.datasets.ImageNet(root_directory, split = 'val')
