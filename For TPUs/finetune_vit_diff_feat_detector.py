# assumes KAMIM to always be there
# Only supported dataset is Imagenet.
...
assert MODEL_SAVE_INTERVAL%SAVE_INTERVAL == 0
...
# %%
# try loop for XLA device
DEVICE = xm.xla_device()

# %%
PROJECT_NAME = f'KAMIM - Other detectors ({process})'

WEIGHT_PATH = f'Models/{dataset}/{model_name}/{algo} - {weight_ps} - {TEMPERATURE} - {detector}/checkpoint_final.pth'
RUN_NAME = f'[{detector}] {model_name} [{algo}] [WP_Size = {weight_ps}] [T = {TEMPERATURE}]'
MODEL_SAVE_PATH = f'Benchmarks/{dataset}/{model_name}/{process}/{algo} - {weight_ps} -{TEMPERATURE} - {detector}/checkpoint'
print(WEIGHT_PATH)
